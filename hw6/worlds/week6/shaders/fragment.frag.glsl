...
vec3 phongShading(vec3 vNor, vec3 uAmbient, vec3 uDiffuse, vec4 uSpecular){
    vec3 color = uAmbient;
    for (int j=0; j<NL; j++){
        vec3 R = 2.*dot(normalize(vNor), normalize(uLights[j].dir))*normalize(vNor) - normalize(uLights[j].dir);
        vec3 E = -(normalize(vec3(vPos.x, vPos.y, -5.)));
        color+=uLights[j].col*(uDiffuse*max(0.,dot(normalize(vNor),normalize(uLights[j].dir)))+uSpecular.xyz*pow(max(0.,dot(E,R)),uSpecular.w));
    }
    return color;
}
